...
const HandCreamItems = ({ item }) => {
    const handCreamItemsRef = useRef(null)
    ...
    useEffect(() => {
        handCreamItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="handCreamItems" ref={handCreamItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="handCreamImage" imageSrc={`${PROJECT_URL}/images/skin/${item.image}`} />
            </Link>
