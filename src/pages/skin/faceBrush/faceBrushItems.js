...
const FaceBrushItems = ({ item }) => {
    const facebrushItemsRef = useRef(null)
    ...
    useEffect(() => {
        facebrushItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="facebrushItems" ref={facebrushItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="facebrushImage" imageSrc={`${PROJECT_URL}/images/skin/${item.image}`} />
            </Link>
