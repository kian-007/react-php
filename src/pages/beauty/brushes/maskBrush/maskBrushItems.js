...
const MaskBrushItems = ({ item }) => {
    const MaskBrushItemsRef = useRef(null)
    ...
    useEffect(() => {
        MaskBrushItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="MaskBrushItems" ref={MaskBrushItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="maskBrushImage" imageSrc={`${PROJECT_URL}/images/beauty/${item.image}`} />
            </Link>
