...
const BhCosmeticBrushItems = ({ item }) => {
    const bhCosmeticBrushItemsRef = useRef(null)
    ...
    useEffect(() => {
        bhCosmeticBrushItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="bhCosmeticBrushItems" ref={bhCosmeticBrushItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="bhCosmeticBrushImage" imageSrc={`${PROJECT_URL}/images/beauty/${item.image}`} />
            </Link>
...
export default BhCosmeticBrushItems;