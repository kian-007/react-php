...
const LampLipGlossItems = ({ item }) => {
    const lampLipGlossItemsRef = useRef(null)
    ...
    useEffect(() => {
        lampLipGlossItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="lampLipGlossItems" ref={lampLipGlossItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="lampLipGlossImage" imageSrc={`${PROJECT_URL}/images/beauty/${item.image}`} />
            </Link>
...
export default LampLipGlossItems;