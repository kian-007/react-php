...
const CapsulLipGlossItems = ({ item }) => {
    const capsulLipGlossItemsRef = useRef(null)
    ...
    useEffect(() => {
        capsulLipGlossItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="capsulLipGlossItems" ref={capsulLipGlossItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="capsulLipGlossImage" imageSrc={`${PROJECT_URL}/images/beauty/${item.image}`} />
            </Link>
...
export default CapsulLipGlossItems;