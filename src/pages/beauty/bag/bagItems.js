...
const BagItems = ({ item }) => {
    ...
    useEffect(() => {
        bagItemsRef.current.classList.add('visible')
    }, [])

    return (
        <li key={item.id} className="bagItems" ref={bagItemsRef}>
            <Link to={`/product/${item.id}`}>
                <Image myid="bagImage" imageSrc={`${PROJECT_URL}/images/beauty/${item.image}`} />
            </Link>
